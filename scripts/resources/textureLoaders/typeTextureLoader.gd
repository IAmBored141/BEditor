...
var frames:int
var textures:Dictionary[int,IndexTextureLoader] = {}
...
func _init(path:String, capitalised:bool=false, _frames:int=1) -> void:
	frames = _frames
	for i in len(types()):
		textures[types()[i]] = (IndexTextureLoader.new(path.replace("$t", typeNames()[i] if capitalised else typeNames()[i].to_lower()),frames))
